...
class Treatment extends BaseLogic
{
    public $alias = "诊疗方案";
    ...
    public function prepareRows()
    {
        ...
        if ($role == "admin") {
            $rows = Db::query("SELECT t.id, d.name doctor, p.name patient, t.create_time
                            FROM treatment t
    ...
    public function loadDetail($id) {
        $data = Db::query("SELECT t.id,t.create_time,d.name doctor,p.name patient,t.description `description`
        FROM treatment t 
        INNER JOIN inpatient_file f
        INNER JOIN doctor d
        INNER JOIN patient p
